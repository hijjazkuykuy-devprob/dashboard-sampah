...
function App() {
  ...
  // MASTER DATA: Daftar Tempat Sampah (Dosen bakal seneng ndelok iki)
  const daftarTempatSampah = [
    {
      id: 'TRASH-01',
      lokasi: 'LAB IoT - UTAMA',
      kapasitas: firebaseKapasitas, // Gantungno langsung teko Firebase
      servo: firebaseServo,         // Gantungno langsung teko Firebase
      isFirebase: true
    },
    {
      id: 'TRASH-02',
      lokasi: 'KORIDOR GEDUNG D3',
      kapasitas: 45, // Data simulasi statis
      servo: 0,
      isFirebase: false
    },
    {
      ...
      kapasitas: 85, // Data simulasi statis (Kondisi Kebek/Abang)
      servo: 1,
      isFirebase: false
    }
  ];

  // Logic nyaring data adhedhasar apa sing diketik user
  const dataTerpilih = daftarTempatSampah.find(
    trash => trash.id.toLowerCase() === searchQuery.trim().toLowerCase()
  ) || daftarTempatSampah[0]; // Yen gak ketemu utawa kosong, default balik nang TRASH-01

  // Fungsi tombol kontrol servo
  const handleKontrol = (nilai) => {
    if (dataTerpilih.isFirebase) {
      ...
      // Gak ngaruh nang Firebase lek milih tong sampah simulasi liyane
    ...
    <div style={containerStyle}>
      <div style={contentWrapper}>
        ...
        <header style={headerStyle}>
          ...
          {/* BAR PENCARIAN SAIKI WIS FUNGSI COK */}
        ...
        {/* SUB-INFO INDIKATOR ID SING LAGI AKTIF */}
        <div style={idIndicator}>
        ...
        <main style={mainGrid}>
          <section style={fixedCard}>
            ...
            <div style={{ ...valueDisplay, color: statusColor }}>{dataTerpilih.kapasitas}%</div>
          ...
          <section style={fixedCard}>
            ...
            <div style={iconBox}>
              {dataTerpilih.servo === 1 ? (
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="100" height="100">
                  <path d="M14 18 L50 18 L46 14 L18 14 Z" fill="none" stroke="#ff4757" strokeWidth="3" strokeLinejoin="round"/>
            ...
            <div style={statusBadge(dataTerpilih.servo)}>{dataTerpilih.servo === 1 ? 'TERBUKA' : 'TERTUTUP'}</div>
...
const contentWrapper = { width: '100%', maxWidth: '1300px', padding: '20px 40px 60px 40px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center' };
const headerStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%', marginBottom: '20px', flexWrap: 'nowrap' };
const headerLeft = { display: 'flex', flexDirection: 'column', alignItems: 'flex-start' };
const mainTitle = { fontSize: '2.6rem', fontWeight: '900', margin: '0 0 8px 0', letterSpacing: '1px' };
...
const searchContainer = { position: 'relative', display: 'flex', alignItems: 'center', marginTop: '10px' };
const searchIcon = { position: 'absolute', left: '14px', fontSize: '14px' };
const searchInput = { background: 'rgba(255, 255, 255, 0.05)', border: '1px solid #38bdf8', borderRadius: '10px', padding: '12px 15px 12px 42px', color: '#ffffff', fontSize: '14px', width: '310px', outline: 'none' };
const idIndicator = { width: '100%', textAlign: 'left', marginBottom: '30px', fontSize: '0.95rem', color: '#a1a1aa' };
const mainGrid = { display: 'grid', width: '100%', gap: '40px', gridTemplateColumns: '1fr 1fr', boxSizing: 'border-box' };
const fixedCard = { background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '28px', padding: '50px 40px', textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxSizing: 'border-box', minHeight: '440px' };
const cardLabel = { fontSize: '0.8rem', color: '#64748b', letterSpacing: '2px', fontWeight: '700', margin: '0 0 20px 0' };
const valueDisplay = { fontSize: '6.5rem', fontWeight: '900', lineHeight: '1', flex: '1', display: 'flex', alignItems: 'center', justifyContent: 'center' };
const iconBox = { flex: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '30px 0' }; 
...
const statusBadge = (s) => ({ width: '100%', padding: '12px 0', borderRadius: '12px', fontSize: '0.95rem', fontWeight: '700', background: s === 1 ? 'rgba(0, 230, 118, 0.08)' : 'rgba(255, 71, 87, 0.08)', color: s === 1 ? '#00e676' : '#ff4757', marginBottom: '20px' });
...
const footerStyle = { marginTop: '100px', color: '#334155', fontSize: '0.75rem', textAlign: 'center', width: '100%' };
